...
| `en_name`         | lastName (when differs from name) | `feishu_en_name`       | English name; mapped to lastName only when different from `name`; otherwise `name` is used for lastName to satisfy Keycloak's default profile |
